...
module.exports.isTime = function(time){
  return(time && time.isTime);
}
...
Time.prototype.fromString = function(time){
  time = time || "";
  time = time.replace(':', '').replace(':', ''); //not sure why it's not replacing all. i tried w/ regex which is suppose to fix it to no avail
