...
Almanac.prototype.next = function(options, callback){
  ...
  if(this.type)
    occasion = nexts[this.type](this, options);
...
var nexts = {
  single : function(almanac, options){
    options = options || {};
    var date = getDate(options.date);
    var endDate = moment(almanac.endDate)
    if(date <= endDate){
      return almanac;
    }
    else{
  ...
  daily : function(almanac, options){ 
    options = options || {}
    var date = getDate(options.date);

    // TODO: should we really be returning new almanacs here?
    var occurrence = new Almanac(almanac);
    occurrence.startDate = date.add('d',1);
    return occurrence;
  }
}
